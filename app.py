...
if "authenticated" not in st.session_state or not st.session_state.authenticated:
    ...
    st.session_state.api_key = ""  # Reset API key if not authenticated
...
if not st.session_state.authenticated:
    ...
    with col2:
        if st.button("Logout"):
            st.session_state.authenticated = False
            st.session_state.api_key = ""  # Resetta l'API key al logout
            st.rerun()
    ...
    # Dopo aver definito il campo per l'API key
    if "api_key" not in st.session_state:
        st.session_state.api_key = os.getenv("GOOGLE_API_KEY", "")

    # Aggiorna il campo di input per l'API key
    api_key = st.text_input(
        "Enter your Gemini API Key:", 
        value=st.session_state.api_key,  # Recupera l'API key salvata
        type="password"
    )

    # Salva l'API key in session_state quando l'utente la modifica
